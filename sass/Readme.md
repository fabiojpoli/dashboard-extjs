...
folder are mapped to the application's root namespace, 'DashboardExtJS'. This is set in
`"app.json"`:

    "sass": {
        "namespace": "DashboardExtJS"
    }
...
included in the application's JavaScript code build. By default, files in this 
folder are mapped to the application's root namespace, 'DashboardExtJS' in the same way
as `"DashboardExtJS/sass/src"`.
